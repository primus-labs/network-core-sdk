  ...
  describe('submitTask method', () => {
    it('should successfully initialize and attempt to submit task on Base Sepolia network', async () => {
      ...
      try {
        ...
        const dataPath = "$.data[0].baseCcy";
        const dataKeyName = "keynametest";
          ...
          [
            {
              ...
              parseType: "json",
              parsePath: dataPath,
              op: "REVEAL_GRUMPKIN_COMMITMENT" // compute commitment
            }
          ]
        ];
        ...
        console.log('Task result:', taskResult);
        {
          ...
          // Get plaintext data corresponding to JSON path 
          const plainResponse = primusNetwork.getPlainResponse(taskId, 0, dataPath);
          console.log('allPlainResponse:', plainResponse);

          // Get the random number corresponding to the commitment.
          const randomData = primusNetwork.getPrivateData(taskId, dataKeyName);
